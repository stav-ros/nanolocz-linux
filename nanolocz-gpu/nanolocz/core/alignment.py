...
def _refine_shift_parabolic(
    ...
    neighborhood : int
        Size of neighborhood for parabolic fit (default 1 for 3x3).
    ...
    # Skip refinement if peak is at edge/corner (insufficient neighborhood)
    if y0 <= neighborhood or y0 >= ny - neighborhood or x0 <= neighborhood or x0 >= nx - neighborhood:
        return dx_coarse, dy_coarse
    
    # Extract 3x3 neighborhood around peak
    y_start = y0 - neighborhood
    y_end = y0 + neighborhood + 1
    x_start = x0 - neighborhood
    x_end = x0 + neighborhood + 1
    
    neighborhood_data = cross_corr[y_start:y_end, x_start:x_end]
    
    if neighborhood_data.size < 9:
        # Not enough points for fit, return coarse estimate
        return dx_coarse, dy_coarse
    
    # Create coordinate grid centered at the peak (0,0 at peak center)
    yy, xx = np.mgrid[y_start:y_end, x_start:x_end]
    xx_centered = xx - x0
    yy_centered = yy - y0
    
    # Fit 2D paraboloid: z = a*x² + b*y² + c*x*y + d*x + e*y + f
    # where x,y are centered at the peak
    X = np.column_stack([
        xx_centered.ravel(), yy_centered.ravel(),
        xx_centered.ravel()**2, yy_centered.ravel()**2,
        xx_centered.ravel() * yy_centered.ravel(),
        np.ones(xx_centered.size)
    ])
    ...
    try:
        ...
        # Vertex location relative to peak center
        dx_offset = -(2 * b * d - c * e) / denom
        dy_offset = -(2 * a * e - c * d) / denom
        
        # Clamp offset to reasonable range (sub-pixel refinement only)
        dx_offset = np.clip(dx_offset, -0.5, 0.5)
        dy_offset = np.clip(dy_offset, -0.5, 0.5)
        
        # Return coarse shift plus sub-pixel offset
        return dx_coarse + dx_offset, dy_coarse + dy_offset
    except Exception:
        return dx_coarse, dy_coarse
